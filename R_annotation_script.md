...
# check how many genes in laevis
# do a unique count of gene names in laevis file
  # lines in file bigger than unique genes (bc 1 gene per exon) prob ~40,000
# 70% should match
# human ~ 25000 prot coding genes
# laevis has ~ double that, but some lost so ~40,000

# raw blast output check how many unique laevis genes have match of any kinds
# this BUT for laevis gene names
#length(unique(L_H$V1))

#length(unique(hum_ann$H_gene_name))
#length(unique(hum_ann$laev_full))
# what proportion of unique allo transcripts match to laevis
...
# add colnames for laev-specific info
...
# make exon length column (not needed)
...
# this bit parses hum/laev file to completion
#############################################################################
...
sex <- subset(A_L, allo_tig == "tig00012378")
AL_now_laev <- unique(sex$laev_full)


sex <- subset(A_L, allo_tig == "tig00006777")
AL_now_laev <- unique(sex$laev_full)
...
#write.table(end_tab, file = "R_again_exon_annotation_final.txt", sep = "\t", row.names = FALSE, col.names = TRUE)
...
# BEST SANITY CHECK SO FAR
length(unique(ann_done$laev_full))
length(unique(hum_ann$laev_full))

# SANITY CHECKS THAT ANNOTATION FILE RAN CORRECTLY

# completed allo annotation
nrow(ann_done)
# parsed L/H
length(unique(final_laev$L_full))
# unparse A/L
length(unique(A_L$V1))

# average exons per contig
length(unique(A_L$V1))/length(unique(A_L$V2))
